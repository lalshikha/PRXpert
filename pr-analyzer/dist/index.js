...
async function main() {
    ...
    const octokit = (0, github_1.createClient)(token);
    const files = await (0, github_1.getChangedFiles)(octokit, owner, repoName, pullNumber);
    const riskFindings = (0, risk_1.calculateRisk)(files.map(f => ({
        filename: f.filename,
...
${riskFindings.length > 0
        ? riskFindings
            .map(f => `- **${f.severity.toUpperCase()}** [${f.category}] ${f.message}${f.file ? ` in \`${f.file}\`` : ''}`)
            .join('\n')
...
`;
    ...
    await (0, github_1.createReview)(octokit, owner, repoName, pullNumber, summary, inlineComments);
