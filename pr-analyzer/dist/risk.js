...
function calculateRisk(files) {
    const findings = [];
    // Security: auth/session files
    const authFiles = files.filter(f => /auth|session|login|password|token|credential/i.test(f.filename));
    ...
    const permissionFiles = files.filter(f => /permission|role|access|policy/i.test(f.filename));
    ...
    const configFiles = files.filter(f => /config|schema|migration|env|database/i.test(f.filename));
    ...
    const uiFiles = files.filter(f => /\.(tsx|jsx|vue|svelte|html)$/i.test(f.filename) &&
